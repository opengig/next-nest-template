...
export class DeepgramClient {
  ...
  createLiveTranscription(options: Partial<TranscriptionOptions> = {}) {
    const transcriptionOptions: any = {
      encoding: options.encoding || this.defaultTranscriptionOptions.encoding,
      sample_rate:
        options.sampleRate || this.defaultTranscriptionOptions.sampleRate,
      channels: options.channels || this.defaultTranscriptionOptions.channels,
      model: options.model || this.defaultTranscriptionOptions.model,
      smart_format:
        options.smartFormat ?? this.defaultTranscriptionOptions.smartFormat,
      language: options.language || this.defaultTranscriptionOptions.language,
      detect_language:
  ...
  async synthesizeAudio(
    ...
    try {
      const response = await this.client.speak.request({ text }, synthOptions);
