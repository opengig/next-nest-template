...
export class OpenAIWrapper {
	...
	constructor(config: OpenAIWrapperConfig = {}) {
		...
		if (serviceProvider === "openai") {
			...
			if (!apiKey) {
				throw new Error(
					"OPENAPI key is required. Set it in environment",
				);
			}
			...
			if (!apiKey) {
				throw new Error(
					"AZURE OPENAI key is required. Set it in environment",
				);
			}
			...
			throw new Error(
				"Invalid service provider. Choose 'openai' or 'azure'.",
			);
		}
	}
	...
	async generateResponse(
		...
		while (retryCount < this.maxRetries) {
			try {
				...
				console.error(
					`API call failed with error: ${error}. Retrying...`,
				);
				await new Promise((resolve) =>
