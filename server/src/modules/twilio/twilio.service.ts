...
export class TwilioService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    this.client.on("start", (data) => this.eventEmitter.emit("start", data));
    this.client.on("media", (data) => this.eventEmitter.emit("media", data));
    this.client.on("stop", (data) => this.eventEmitter.emit("stop", data));
    this.client.on("close", (data) => this.eventEmitter.emit("close", data));
    this.client.on("error", (error) => this.eventEmitter.emit("error", error));
  }
  ...
  on<K extends keyof TwilioEvents>(event: K, listener: TwilioEvents[K]): void {
    this.eventEmitter.on(event, listener);
  }

  off<K extends keyof TwilioEvents>(event: K, listener: TwilioEvents[K]): void {
